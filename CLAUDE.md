# Humm — Sinus Health Through Sound

Humm is an iOS app that guides users through a science-backed humming technique to relieve sinus congestion. It uses real-time microphone input to provide ambient visual feedback through a Metal shader, helping users hum at the correct pitch (~130 Hz) and strength without clinical-feeling meters or numbers.

**Think:** Apple Mindfulness meets a musical tuner — but the interface communicates entirely through a living, breathing visual form.

## Key Domain Context
- Humming at ~130 Hz increases nasal nitric oxide 15x (Weitzberg & Lundberg, 2002)
- **Phase 1 (Initial):** 4 days, ~1 hour daily of "good hum time"
- **Phase 2 (Maintenance):** Ongoing, ~10 minutes daily
- "Good hum time" = time spent in acceptable pitch + amplitude range
- Timer accumulates only during good hum time, pauses naturally

## Design Principles
1. **Ambient over clinical** — visual form, not numbers or meters
2. **Beauty sustains attention** — captivating for hour-long sessions
3. **Simplicity and clarity** — Apple Mindfulness-inspired, minimal UI
4. **Progressive disclosure** — science in bite-sized pieces
5. **Low pressure** — gentle visual fade, not failure states

## Build Order (from PRD)
1. Audio engine (AVAudioEngine — pitch detection + amplitude)
2. Shader (Metal cymatic-inspired mesh gradient responding to audio)
3. Session screen (shader + progress + stop button)
4. Home screen (day circles, start button, phase 1/phase 2)
...
This is a native **iOS application** built with **Swift 6.1+** and **SwiftUI**. The codebase targets **iOS 17.0 and later**. All concurrency is handled with **Swift Concurrency** (async/await, actors, @MainActor isolation) ensuring thread-safe code.

- **Frameworks & Tech:** SwiftUI for UI, AVAudioEngine for real-time audio, Metal for shaders, Swift Concurrency with strict mode, Swift Package Manager for modular architecture
...
- **Accessibility:** Full accessibility support — feedback encoded in shape/size (not color) for colorblind support
...
- **Entitlements File**: `Config/Humm.entitlements` contains all app capabilities
...
To add capabilities to your app, edit `Config/Humm.entitlements`:
